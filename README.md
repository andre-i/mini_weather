...
Важно: __при напряжении больше 3.3В модуль может сгореть!!!__
...
+ В ArduinoIDE 
  ...
  + Скачать или клонировать этот репозиторий(зелёная кнопка вверху на ней написано "clone or download") в любую удобную директорию на своём компьютере
  ...
  + Подключить модуль
  + Подать напряжение на схему( Конвертер в USB порт). Во время запуска держать кнопку Flash нажатой, после можно отпустить.
  ...
  + После окончания загрузки запустить *serial monitor* ArduinoIDE с параметрам *no line ending* и скоростью 115200
  ...
  + В консольной справке указано как можно настроить стартовые параметры и посмотреть текущие настройки. Справку смотрят в *serial monitor*, для этого достаточно отправить 'ru' в мониторе.


### Подключение к модулю
Подключение компьютера к модулю для работы с ним заключается в подключении USB-TTL конвертера. Соединить RX конвертера с TXD0 модуля, TX конвертера с RXD0 модуля и земли(gnd) модуля и конвертера. Смотри на схеме.
...
+ Выбирать период начинают с года далее, если необходимо, уточняем месяц, число.
...
+ **Некоторые настройки**. Для настройки используется последовательный порт путём отправки разных команд. Для этого надо подключиться к модулю(как подключиться смотри выше). После того, как соединение установлено шлём модулю *ru* и читаем как и что делается. Ниже некоторые настройки и возможности работы через последовательный порт.
...
*Почти все изменения вступают в силу после перезагрузки.*
После того, как время установлено **не перезагружать модуль** иначе установленное время будет сброшено.
